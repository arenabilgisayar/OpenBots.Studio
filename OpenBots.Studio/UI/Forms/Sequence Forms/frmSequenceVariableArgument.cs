            ...
            try
            {
                ...
                if (e.ColumnIndex == 0)
                {
                        ...
                        (cellValue != null && !provider.IsValidIdentifier(cellValue.ToString())))
                    {
                        ...
                        await StudioVariableMethods.RemoveVariable(_preEditVarArgName, ScriptContext);
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        await StudioVariableMethods.AddVariable(nameCell.Value.ToString(), (Type)typeCell.Value, valueCell.Value?.ToString(), ScriptContext);
                    }
                }
                ...
                else if (e.ColumnIndex == 2)
                {
                    try
                    {
                        await StudioVariableMethods.UpdateVariable(nameCell.Value.ToString(), (Type)typeCell.Value, valueCell.Value?.ToString(), ScriptContext);
        ...
        private async void dgvVariablesArguments_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    await StudioVariableMethods.RemoveVariable(e.Row.Cells[0].Value.ToString(), ScriptContext);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void dgvVariablesArguments_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex != -1)
                {
                    ...
                    await StudioVariableMethods.RemoveVariable(nameCell.Value.ToString(), ScriptContext);
